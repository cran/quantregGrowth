...
Since version 1.6-0, it is possible to set an adaptive penalty via the weighted lasso, where the weights are updated iteratively. `ad=1`specified in `ps()` leads to the adaptive lasso 
...
Note each symbol refers to a specific quantile curve, and results from the adaptive lasso are closer to the true ones.
