...
#        fit<-fitter(y=Y,X=X,tau=tau,g=g,beta0=b.start,control=control)
      fit<-ncross.rq.fitX(y=Y, X=X, taus=tau) #X già include l'intercetta
      #class(fit)<-"cgrq"
    } else {
    #se ci sono..
    #se non ci sono termini ps (length(testo.ps)==0) crea questi oggetti che vengono richiamati se length(testo.ridge)>0
...
#        z<-with(mf,eval(parse(text=testo.ps))) #questo va bene se ce ne solo uno di testo.ps
        ...
        for(j in 1:ncol(mVariabili)) {
          B[[length(B)+1]]<- bspline(mVariabili[,j],ndx=vNdx[j],deg=vDeg[j]) 
          #---------?????????????????????????
          ...
          Bderiv<-bspline(xdisegno,ndx=vNdx[j], deg=vDeg[j], deriv=1)
